...
import { useHelpRequests, useMissionMessages, useAcceptHelp, useDenyHelp, useCompleteFlare, useSendHelpChatMessage } from '@/hooks/useHelpRequests'
...
export function MessagesView() {
  ...
  const { data: rawFlares = [] } = useFlares()
  const { data: helpRequests = [] } = useHelpRequests()
  const { data: messages = [] } = useMissionMessages()
  
  const acceptHelpMutation = useAcceptHelp()
  const denyHelpMutation = useDenyHelp()
  const completeFlareMutation = useCompleteFlare()
  const sendHelpMessageMutation = useSendHelpChatMessage()
  const markAsReadMutation = useMarkMessagesAsRead()
  
  const [searchParams] = useSearchParams()
  const initialCircleChatUserId = searchParams.get('circleChat')
  
  if (!authUser || !profile) return null

  // Build the user data object matching the old User type
  const user = {
    id: authUser.id,
  ...
  // Define handler wrappers
  const onAcceptHelp = (id: string) => acceptHelpMutation.mutate(id)
  const onDenyHelp = (id: string) => denyHelpMutation.mutate(id)
  const onCompleteFlare = (flareId: string, helperId: string) => completeFlareMutation.mutate({ flareId, helperId })
  const onSendMessage = (helpRequestId: string, content: string) => sendHelpMessageMutation.mutate({ helpRequestId, content, helpRequests })
  const onMarkAsRead = () => markAsReadMutation.mutate()
  const onUserClick = undefined // Could implement user modal but currently handled globally by AppLayout

  const flares = rawFlares.map((f: Record<string, unknown>) => {
    const profiles = f.profiles as { display_name?: string } | null
    return {
      ...f,
      location: f.location as { lat: number; lng: number } | null,
      creator_name: profiles?.display_name || 'Onbekende buur',
    }
  }) as Flare[]
